...
unless defined?(Kconv)
  module Kconv
    AUTO = 0; EUC = 1; SJIS = 2; UTF8 = 3; BINARY = 4; ASCII = 5; JIS = 6
    def self.guess(str)
      return BINARY if str.nil? || str.empty?
      return UTF8 if str.encoding == Encoding::UTF_8 && str.valid_encoding?
    ...
    def self.toeuc(str)
      return str if str.nil?
      str.encode(Encoding::EUC_JP, invalid: :replace, undef: :replace) rescue str.encode(Encoding::UTF_8, invalid: :replace, undef: :replace)
    end
    def self.tosjis(str)
      return str if str.nil?
      str.encode(Encoding::Shift_JIS, invalid: :replace, undef: :replace) rescue str.encode(Encoding::UTF_8, invalid: :replace, undef: :replace)
    end
  end
end
